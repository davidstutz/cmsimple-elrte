...
elRTE.prototype.ui.prototype.buttons.elfinder = function(rte, name) {
	...
	this.command = function() {
		if (self.rte.options.fmAllow && typeof(self.rte.options.fmOpen) == 'function') {
			...
			}, self.rte.options.folders.elfinder );
		}
	}
				...
				var src = $('<span />').append(this.src.url.css('width', '85%'))
						.append(
							$('<span />').addClass('ui-state-default ui-corner-all')
								...
								.append($('<span />').addClass('ui-icon ui-icon-folder-open').attr('id', 'flash'))
									.click( function() {
										self.rte.options.fmOpen( function(url) { self.src.url.val(url).change(); }, self.rte.options.folders.flash );console.log(self.rte.options.folders.flash);
									})
	...
	this.command = function() {
		...
		if (fm) {
			src.children('.ui-state-default')
				.click( function() {
					rte.options.fmOpen( function(url) { self.src.main.src.val(url).change() }, self.rte.options.folders.image );console.log(self.rte.options.folders.image);
				})
				.hover(function() {
				...
				.append(
					$('<span />').addClass('ui-state-default ui-corner-all')
						...
						.append($('<span />').addClass('ui-icon ui-icon-folder-open'))
							.click( function() {
								self.rte.options.fmOpen( function(url) { self.src.main.href.val(url).change(); }, self.rte.options.folders.link );
							})
							.hover(function() {$(this).addClass('ui-state-hover')}, function() { $(this).removeClass('ui-state-hover')})
				);
